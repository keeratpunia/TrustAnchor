            ...
            <Card key={a.id} style={{ padding: '16px 18px' }}>
              ...
              {a.status === 'APPROVED_NO_KEY' && (
                <div style={{ marginTop: 14 }}>
                  {a.pendingPublicKeyHex ? (
                    <>
                      <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 'var(--radius-sm)', padding: '12px 16px', marginBottom: 14 }}>
                        <div style={{ fontWeight: 600, fontSize: 13, color: '#166534', marginBottom: 4 }}>
                          Issuer has submitted their public key
                        </div>
                        <div style={{ fontSize: 12, color: '#15803d' }}>
                          (This public key can be trusted. For added security, verify this matches what the issuer generated by confirming the first and last few characters from the issuer itself.)
                        </div>
                      </div>
                      <div style={{ background: 'var(--parchment-050)', border: '1px solid var(--hairline)', borderRadius: 'var(--radius-sm)', padding: '12px 16px', marginBottom: 14 }}>
                        <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--slate-500)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 6 }}>Submitted Public Key</div>
                        <div className="mono" style={{ fontSize: 12, wordBreak: 'break-all', lineHeight: 1.6, color: 'var(--ink-900)' }}>
                          {a.pendingPublicKeyHex}
                        </div>
                      </div>
                      {!expanded ? (
                        ...
                        <PublishKeyForm
                          submitLabel="Confirm Key & Activate"
                          initialPublicKeyHex={a.pendingPublicKeyHex}
                          onSubmit={(v) => handlePublishKey(a.id, v)}
                        />
                      )}
                    </>
                  ) : (
                    <>
                      <div style={{ fontSize: 12.5, color: 'var(--slate-500)', marginBottom: 10 }}>
                        The issuer hasn't submitted their public key yet. You can wait, or enter it manually if you received it another way.
                      </div>
                      {!expanded ? (
                        <Button variant="secondary" onClick={() => setExpandedId(a.id)}>
                          Enter Key Manually
                        </Button>
                      ) : (
